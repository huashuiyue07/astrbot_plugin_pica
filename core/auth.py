...
class PicaAuthManager:
    ...
    def status(self, user_id: str) -> dict:
        ...
        if bound_token:
            return {
                "bound": True,
                "source": "self",
                "email": None,  # token 无法反解邮箱，仅标记已绑定
    ...
    async def force_relogin(self, user_id: str) -> str:
        ...
        """
        # 1. 用户自绑账号：用缓存的 email/password 重登
        if user_id and user_id != "__default__":
            self.logout(user_id)
            if self.email and self.password:
                token = await self.client.login(self.email, self.password)
                self._save_token(self._token_path(user_id), token)
